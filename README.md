...
- Унифицированный стиль вкладок по аналогии с awg-manager
...
- Новые DNS-группы создаются с префиксом `_NCM<N>` вместо `_p<N>`
- Существующие группы `_p<N>` и без префикса не трогаются

## Изменения в версии 1.9.3

- DNS-маршруты выделены из `peers.json` в отдельный файл `dns-routes.json`
- При первом запуске DNS-маршруты автоматически мигрируют из `peers.json`

## Изменения в версии 1.9.0–1.8.0

- Светлая/тёмная тема с переключателем в заголовке (как awg-manager)
- Тех overhaul заголовка и вкладок по стилю awg-manager
- Унифицированный стиль всех карточек вкладок
- В заголовке «Пиры» отображается интерфейс, IP, общее количество пиров и неоплаченных
- Индикатор оплаты пиров (красный/зелёный `$`)
- Полный бэкап/рестор в ZIP со всеми данными, atomic writes
- Автозапуск AmneziaWG через systemd
- Исправление транслитерации кириллицы в именах пиров
- Исправление смены подсети: принудительный `ip link del dev wg0`, перезапись wg0.conf
- Затенение групповых пресетов DNS при добавлении
